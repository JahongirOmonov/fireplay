...
class today(APIView):
    def get(self, request, *args, **kwargs):
        ...
        x=todoModel.objects.filter(crete_at__date=today)
...
class lastTen(APIView):
    def get(self, request, *args, **kwargs):
        ...
        x=todoModel.objects.filter(crete_at__gte=last_ten, crete_at__lte=today)
        serializer=TodoSerializer(x, many=True)
        return Response(serializer.data)
...
class status(APIView):
    def get(self, request, *args, **kwargs):
        today=datetime.date.today()
        x=todoModel.objects.filter(crete_at__date=today, status=True)
        serializer=TodoSerializer(x, many=True)
        return Response(serializer.data)
